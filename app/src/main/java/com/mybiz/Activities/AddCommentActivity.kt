...
import com.mybiz.R
import com.mybiz.extensions.setupToolbar
...
class AddCommentActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        vote_name.text = getSharedPreferences(Constants.PREFERENCES, MODE_PRIVATE).getString(Constants.NAME, Constants.RANDOM_STRING)
        ratingBar.rating = 1
        ratingBar.rating = intent?.getIntExtra("ratingAmount", 0)?.minus(1) ?: 0

        vote.setOnClickListener {
            FirebaseDatabase.getInstance().reference.child("AllUsers").child("PrivateData").child(getSharedPreferences(Constants.PREFERENCES, MODE_PRIVATE).getString(Constants.APP_ID, Constants.RANDOM_STRING)!!).child("profileUrl").addValueEventListener(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    val userUrl = snapshot.value as String
                    val comment = Comment().apply {
                        writer = getSharedPreferences(Constants.PREFERENCES, MODE_PRIVATE).getString(Constants.NAME, Constants.RANDOM_STRING)
                        writerUid = getSharedPreferences(Constants.PREFERENCES, MODE_PRIVATE).getString(Constants.APP_ID, Constants.RANDOM_STRING)
